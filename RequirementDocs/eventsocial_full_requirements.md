...
### 4.2 Artist: Create → Promote
1. Applies for verification → admin approves.
2. Creates event (draft) → sets date/time/venue → publishes.
3. Uploads reels and links to event.
4. Monitors insights (views → clicks → bookings).

### 4.3 Admin: Trust & Operations
1. Verifies artists.
2. Approves/creates events, ensures venue data quality.
3. Reviews reports and removes harmful content quickly.
4. Exports event and user reports for operations.
5. (Phase 2) handles cancellation/refund workflows.
...
### 5.4.3 “Event Storyline” (content hub)
...
- Draft → Preview → Publish (or submit for approval)

**FR-E4** Event publishing rules:
- Only verified artists can publish
- Optional: admin approval required (configurable per region)
...
## 8.2 Auto-hide rule (recommended)
...
### Content
- `reels` (id, artist_id, video_url, cover_url, caption, tags, linked_event_id, status, created_at)
...
- Event creation + publish (with optional approval)
...
1. **Event approval workflow:** direct publish vs admin approval.

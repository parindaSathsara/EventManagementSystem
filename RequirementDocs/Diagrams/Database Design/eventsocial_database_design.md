...
- `ref`: lookup/config tables (`roles`, `reaction_types`, `event_categories`, etc.)
- `iam`: identity, roles, auth identities, devices, saved locations
- `artist`: artist profile + verification workflow
- `media`: storage metadata for video/image/document assets
- `venue`: geospatial venues (`geography(point, 4326)` with GiST index)
- `events`: events, lineup, approvals, change logs, featured reels
- `social`: reels, tags, reactions, reposts, follows, saves, negative signals
...
### 3.4 Strict Report Targets Without Weak Polymorphism
...
- Reels/social actions (no comments)
